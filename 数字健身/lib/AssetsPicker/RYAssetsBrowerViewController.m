...
//    [buttonBack setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/back_arrows.png"] forState:UIControlStateNormal];
//    [buttonBack setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/back_arrows_highlighted.png"] forState:UIControlStateHighlighted];
//    [buttonBack addTarget:self action:@selector(back:) forControlEvents:UIControlEventTouchUpInside];
    ...
    RYAsset *ryAsset=self.arrayAsset[self.currentIndex];
    if (ryAsset.selected) {
        [buttonSelect setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/check.png"] forState:UIControlStateNormal];
    }else{
        [buttonSelect setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/check_no.png"] forState:UIControlStateNormal];
    }
    ...
    [buttonOriginal setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/original_no.png"] forState:UIControlStateNormal];
    ...
    if (self.selectCount==0) {
        ...
        title=[NSString stringWithFormat:@"%@(%ld)",self.titleButtonSure,(long)self.selectCount];
    }
...
- (void)initialScrollView
{
    ...
    NSInteger indexStart=(self.currentIndex-1)<0?0:(self.currentIndex-1);
    NSInteger indexEnd=(self.currentIndex+1)>=self.arrayAsset.count?self.currentIndex:(self.currentIndex+1);
    ...
    //显示当前所选择的图片
    NSInteger indexCount=self.currentIndex-indexStart;
...
- (void)selectAsset:(UIButton *)sender
{
    ...
    RYAsset *ryAsset=self.arrayAsset[subSV.tag-TAG_START];
    ...
    if (nowSelected!=ryAsset.selected) {
        ...
        if (_original) {
            long long newSize=[ryAsset.asset defaultRepresentation].size;
            if (ryAsset.selected) {
...
- (void)selectOriginal:(UIButton *)sender
{
    ...
    _original=!_original;
    if (_original) {
        for (RYAsset *ryAsset in self.arrayAsset) {
...
//                NSLog(@"%lld,%d",[asset defaultRepresentation].size,data.length);
        ...
        if (_originalSize==0) {
            int index=_scrollView.contentOffset.x/_scrollView.frame.size.width;
            RYAsset *ryAsset=self.arrayAsset[index];
            ryAsset.selected=YES;
            self.selectCount++;
            self.originalSize=[ryAsset.asset defaultRepresentation].size;
        }else{
            self.originalSize=_originalSize;
        }
    }else{
        [sender setTitle:@"原图" forState:UIControlStateNormal];
        [sender setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/original_no.png"] forState:UIControlStateNormal];
    }
}
#pragma mark 发送选择图片
...
- (void)setSelectCount:(NSInteger)selectCount
{
    if (self.selectCount<selectCount) {
        [_buttonSelect setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/check.png"] forState:UIControlStateNormal];
    }else{
        [_buttonSelect setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/check_no.png"] forState:UIControlStateNormal];
    }
...
- (void)setOriginalSize:(long long)originalSize
{
    ...
    [_buttonOriginal setImage:[UIImage imageNamed:@"RYAssetsPicker.bundle/original.png"] forState:UIControlStateNormal];
...
- (void)switchView:(UIScrollView *)scrollView
{
    ...
    //显示的图片在资源中的索引
    NSInteger indexAsset=subScrollView.tag-TAG_START;
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    NSLog(@"%ld,%lf",(long)scrollView.tag,_beginDraggingContentOffsetX);
}
