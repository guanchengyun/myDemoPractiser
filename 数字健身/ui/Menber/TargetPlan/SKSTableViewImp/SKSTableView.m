...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        if (cell.isExpandable) {
            ...
            UIButton *expandableButton = (UIButton *)[cell.contentView viewWithTag:100];
            [expandableButton addTarget:tableView
                                 action:@selector(expandableButtonTouched:event:)
                       forControlEvents:UIControlEventTouchUpInside];
            ...
            if (cell.isExpanded) {
                ...
                expandableButton.transform = CGAffineTransformMakeRotation(M_PI_2);
                
//                [self tableView:tableView reloadIndexPath:indexPath];
...
//        
//        [self removeforSection:indexPath.section];
//        [tableView deleteSections:[NSIndexSet indexSetWithIndex:section] withRowAnimation:UITableViewRowAnimationTop];
   
    }else{
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:didSelectForSubRowAtIndexPath:)]){
            NSIndexPath *indexPathForSubrow = [self correspondingIndexPathForSubRowAtIndexPath:indexPath];
            [_SKSTableViewDelegate tableView:tableView commitEditingStyle:editingStyle forSubRowAtIndexPath:indexPathForSubrow];
//            NSMutableArray *indexPaths = [NSMutableArray array];
//            NSInteger row = indexPathForSubrow.subRow;
//            NSInteger section = indexPathForSubrow.section;
//            
...
//              NSLog(@"indexPath::%d indexPath:%d",indexPath.section,indexPath.row);
//            for (NSIndexPath *index in self.expandedIndexPaths[indexPath.section]) {
//                NSLog(@"index::%d index:%d",index.section,index.row);
//            }
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:didSelectForSubRowAtIndexPath:)]){
            ...
            [_SKSTableViewDelegate tableView:(SKSTableView *)tableView didSelectForSubRowAtIndexPath:indexPathForSubrow];
        }
    }
...
-(void)tableView:(UITableView *)tableView targetOpenButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell isKindOfClass:[SKSTableViewCell class]] && cell.isExpandable) {
        ...
        if (cell.isExpanded) {
            ...
            NSInteger delenumberOfSubRows = [self.expandedIndexPaths[indexPath.section]count];
            
            if (delenumberOfSubRows >0) {
                NSMutableArray *indexPaths = [NSMutableArray array];
                NSInteger row = indexPath.row;
                NSInteger section = indexPath.section;
                
                for (NSInteger index = 1; index <= delenumberOfSubRows; index++) {
        ...
        UIButton *expandableButton = (UIButton *)[cell viewWithTag:100];
        [UIView animateWithDuration:0.2 animations:^{
            if (cell.isExpanded) {
                ...
                expandableButton.transform = CGAffineTransformMakeRotation(0);
            }
...
-(void)tableView:(UITableView *)tableView reloadIndexPath:(NSIndexPath *)indexPath{
    NSInteger numberOfSubRowsexpanded = [self.expandedIndexPaths[indexPath.section]count];
    NSInteger numberOfSubRows = [self numberOfSubRowsAtIndexPath:indexPath];
    if (numberOfSubRowsexpanded < numberOfSubRows) {
        NSMutableArray *indexPaths = [NSMutableArray array];
        NSInteger row = indexPath.row;
        NSInteger section = indexPath.section;
        NSIndexPath *expIndexPath = [NSIndexPath indexPathForRow:row+numberOfSubRows inSection:section];
        [indexPaths addObject:expIndexPath];
        [self insertExpandedIndexPaths:indexPaths forSection:indexPath.section];
        [tableView insertRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationTop];
    }
...
-(void)tableView:(UITableView *)tableView insertIndextPath:(NSIndexPath *)indexPath {
    NSInteger numberOfSubRows = [self numberOfSubRowsAtIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:accessoryButtonTappedForSubRowWithIndexPath:)])
            [_SKSTableViewDelegate tableView:tableView accessoryButtonTappedForSubRowWithIndexPath:indexPath];
    }
}
...
- (IBAction)expandableButtonTouched:(id)sender event:(id)event
{
    ...
    if (indexPath){
        if (btn.tag == 100) {
            [self tableView:self targetOpenButtonTappedForRowWithIndexPath:indexPath];
        }else if(btn.tag == 101){
...
- (void)removeExpandedIndexPaths:(NSArray *)indexPaths forSection:(NSInteger)section
{
    ...
    }else if((indexPaths.count == 1)&&([self.expandedIndexPaths[section] count]>0)){
        __block NSMutableArray *array = [NSMutableArray array];
        [self.expandedIndexPaths[section] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            NSIndexPath *updated =nil;
            if ([obj row] <[indexPaths[0] row]) {
                updated = [NSIndexPath indexPathForRow:([obj row])
                                                          inSection:[obj section]];
            }else{
                updated = [NSIndexPath indexPathForRow:([obj row] - [indexPaths count])
                                             inSection:[obj section]];
            }
...
- (void)removeforSection:(NSInteger)section
{
    if (self.expandedIndexPaths.count >= section) {
        [self.expandedIndexPaths removeObjectAtIndex:section];
    }
}
...
static NSString *subRowObjName = @"2014年6月22日4：34";
...
- (NSInteger)subRow
{
//    NSInteger interden = 3;
//    NSLog(@"interden::%d",interden);
    id subRowObj = objc_getAssociatedObject(subRowObjName, SubRowObjectKey);
...
- (void)setSubRow:(NSInteger)subRow
{
    ...
    if (self) {
         objc_setAssociatedObject(subRowObjName, SubRowObjectKey, subRowObj, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
